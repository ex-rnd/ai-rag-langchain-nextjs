...
export async function POST(req: Request) {
  try {
    ...
    const inputMapper = RunnableMap.from<Input, Output>({
      question: (input) => input.question,
      chat_history: (input) => input.chat_history,
      context: () => formatDocumentsAsString(docs as any),
    });

    // ---- FIXED: RunnableSequence now type‑safe ----
